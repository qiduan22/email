...
def hist(_data):
	...
	for i in range(total):
		bin_loc = math.floor(_data[i] * 10)
		bin[bin_loc] += 1
	for i in range(len(bin)):
		bin[i] = float(bin[i]/total)
	return bin

#all = np.loadtxt('all.txt')
b_all = np.loadtxt('b_all.txt')
m_all = np.loadtxt('m_all.txt')
train_data = []
...
test_x = []
for i in range(TRAIN_TOTAL, 2*TRAIN_TOTAL):
	test_x_item = []
	for j in range(len(b_all[i])):
...
for i in range(128):
	#feature = []
	feature = [int(j) for j in bin(i)[2:].zfill(7)]
	sender_test.append(feature)
#print(all_input[0])


#train_cnn_network('all', train_data, train_y, test_x, test_y, 'test',  False)




ratio = test_cnn_run_main('all_bit', sender_test, 'test')
...
for _k, _v in feature_dict.items():
	for __k in _k:
		print(__k, ',', end='')
	print(_v)
print(feature_dict)
...
#print(ratio)
#test_cnn_run_main('mal', test_x, 'test')
